...
fn compress_with_dict(data: &[u8], dict: &[u8], level: u32) -> Vec<u8> {
    ...
    compress.compress(data, &mut output, FlushCompress::Finish).unwrap();
...
fn main() {
    ...
    println!("Block size: {}KB, Dictionary: {}KB\n", block_size/1024, dict_size/1024);
    ...
    for level in [1, 6, 9] {
        ...
        let dict_benefit = (total_no_dict as f64 - total_with_dict as f64) / total_no_dict as f64 * 100.0;
        let forward_benefit = (total_no_dict as f64 - total_estimated_forward as f64) / total_no_dict as f64 * 100.0;
        
        println!("  No dictionary:    {} bytes", total_no_dict);
        println!("  With dictionary:  {} bytes ({:+.1}%)", total_with_dict, -dict_benefit);
        println!("  Forward-refs est: {} bytes ({:+.1}%)", total_estimated_forward, -forward_benefit);
        println!("  Hash priming benefit: {:.1}% (of {:.1}% total dict benefit)", 
