...
fn main() {
    ...
    for l in [1u32, 2, 4, 6, 9] {
        ...
        let (t_ours, s_ours) = best(
            || {
                buf.clear();
                gzippy::compress::ldx::compress_into(l, &d, &mut buf);
                buf.len()
            },
            7,
        );
        let (t_c, s_c) = best(
            || {
                let mut c = libdeflater::Compressor::new(
                    libdeflater::CompressionLvl::new(l as i32).unwrap(),
                );
        ...
        println!(
            "{name:<14} L{l:<2} ldx={:>8.2}ms C={:>8.2}ms  ldx/C={:>5.3}x  bytes {}",
            t_ours * 1000.0,
            t_c * 1000.0,
            t_ours / t_c,
            if s_ours == s_c {
