...
neurotic  ── bare-metal Proxmox HOST, root shell
   ...
   │  ssh root@REDACTED_IP   (one hop, from the host)
   ▼
guest 199 "trainer"  ── LXC container, /root/gzippy lives here

- ssh -J neurotic root@REDACTED_IP = laptop → guest in one command (ProxyJump through the host).
...
ssh -J neurotic root@REDACTED_IP 'cd /root/gzippy && git fetch -q && git reset --hard -q origin/<branch> && cargo test --release ...'
