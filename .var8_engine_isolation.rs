...
mod bench {
    ...
    // ── Variant (viii): FULL-KERNEL inline-asm clean-tail (igzip loop_block) ────
    //
    // VAR_VIII keeps the ENTIRE hot loop in ONE `core::arch::asm!` block with the
    // back-edge (`jmp 2b`) INSIDE the asm: bit-state (read_in / read_in_length /
    // next_in_pos) + output cursor (next_out) + the lit/len short-table base + the
    // output base are PINNED in registers ACROSS the back-edge. Unlike VAR_VII —
    // which exited to Rust on EVERY length code (the DIS-1 per-back-ref re-entry
    // that spilled 4 regs to `bits` ×300-460K/chunk) — VAR_VIII resolves the
    // back-ref ENTIRELY in asm: F2 speculative DIST gather (ISA-L small dist LUT,
    // `LutDistCode.table.short_code_lookup`, u16[1<<10]) + D the overlap-doubling
    // copy, then `jmp 2b` with NO memory write-back. Rust is entered ONLY at a long
    // lit/len code, a long DIST code, EOB (once per block), or the output/input
    // boundary (once near target_end). This is the construct the phase2 advisor
    // OWED and that DIS-1 does NOT bind (DIS-1 measured the per-symbol re-entry).
    //
    // Cold loop-invariants (in_ptr, dist tables, limits) live in a `KernCtx` struct
    // accessed via `[ctx + off]` memory operands, so register pressure stays at 12
    // operands (VAR_VII used 13). The bit engine (refill/consume) mirrors `Bits`
    // EXACTLY (consume_first_decode.rs:245-300) so the byte-exact gate vs VAR_I
    // scalar AND VAR_III ISA-L holds — every byte the asm emits is decoded
    // identically to `LutLitLenCode::decode` / `LutDistCode::decode`.
    //
    // PRE-REGISTERED GATE: PASS iff byte-exact AND asm_frac >= 0.97 (GZIPPY_VIII_
    // COVERAGE) AND VAR_VIII/VAR_III >= 0.85 AND materially > VAR_VI (~0.55-0.60).
    // KILL/PLATEAU iff VAR_VIII ~= VAR_VI (register pinning cannot beat LLVM here).
    #[cfg(target_arch = "x86_64")]
    #[repr(C)]
    struct KernCtx {
        in_ptr: u64,     // +0   deflate.as_ptr()
        dist_tbl: u64,   // +8   dist short_code_lookup (u16[1<<10])
        dist_base: u64,  // +16  DISTANCE_BASE  (u16[30])
        dist_extra: u64, // +24  DISTANCE_EXTRA (u8[30])
        out_limit: u64,  // +32  next_out boundary (= target_end)
        in_limit: u64,   // +40  next_in_pos boundary (= in_end - IN_SLOP)
    }
    ...
    fn decode_var_viii(
        ...
        let cov = std::env::var_os("GZIPPY_VIII_COVERAGE").is_some();
        let mut asm_bytes: u64 = 0;
        let mut tail_bytes: u64 = 0;
        let mut reentries: u64 = 0;
        ...
        'blocks: loop {
            ...
            let short_tbl = litlen.table.short_code_lookup.as_ptr();
            let ctx = KernCtx {
                in_ptr: deflate.as_ptr() as u64,
                dist_tbl: dist.table.short_code_lookup.as_ptr() as u64,
                dist_base: DISTANCE_BASE.as_ptr() as u64,
                dist_extra: DISTANCE_EXTRA.as_ptr() as u64,
                // out_limit = target_end: the asm has out_slop(+4096) of headroom
                // past target_end so its speculative over-writes stay in-bounds; the
                // careful tail finishes the (tiny) remainder to exactly target_end.
                out_limit: target_end as u64,
                in_limit: (in_end - IN_SLOP) as u64,
            };
            let ctx_ptr: *const KernCtx = &ctx;

            'asm_reentry: loop {
                let ret: u64;
                let out_pos_before = out_pos;
                if cov {
                    reentries += 1;
                }
                {
                    let mut read_in: u64 = bits.bitbuf;
                    let mut ril: i64 = bits.bitsleft as i64;
                    let mut next_in_pos: u64 = bits.pos as u64;
                    let mut next_out: u64 = out_pos as u64;
                    unsafe {
                        core::arch::asm!(
                            // Exit codes are set INLINE into `ret` before each jump
                            // to the single end label `9` (labels `0`/`1` are banned
                            // by the binary_asm_labels lint, so the per-exit labels
                            // are folded into speculative `mov {ret}, ..`; `mov`
                            // preserves flags so it is legal between cmp and jcc, and
                            // a stale `ret` on a non-exit path is overwritten by the
                            // real exit or ignored on the `jmp 2b` loop-back).
                            // ret&7: 0=EOB 1=long-litlen/invalid 2=boundary
                            //        3=long-dist(len=ret>>3) 4=invalid len/dist.
                            // ===== top guard (igzip asm:508-512): one check/iter =====
                            "2:",
                            "mov {ret}, 2",                      // speculative: boundary
                            "mov {t2}, qword ptr [{ctx} + 32]",  // out_limit
                            "cmp {next_out}, {t2}",
                            "jae 9f",
                            "mov {t2}, qword ptr [{ctx} + 40]",  // in_limit
                            "cmp {next_in_pos}, {t2}",
                            "jae 9f",
                            // ===== F3 unconditional refill (mirror Bits::refill) =====
                            "mov {t2}, qword ptr [{ctx} + 0]",   // in_ptr
                            "mov {tmp}, qword ptr [{t2} + {next_in_pos}]",
                            "shlx {tmp}, {tmp}, {ril}",
                            "or {read_in}, {tmp}",
                            "mov {cnt}, 63",
                            "sub {cnt}, {ril}",
                            ...
                            "add {next_in_pos}, {cnt}",
                            "or {ril}, 56",
                            // ===== F1 lit/len gather: cnt = short[read_in & 0xFFF] ====
                            "mov {tmp}, {read_in}",
                            "and {tmp}, 0xFFF",
                            "mov {cnt:e}, dword ptr [{short_tbl} + {tmp}*4]",
                            "mov {ret}, 1",                      // speculative: long/invalid litlen
                            "test {cnt:e}, 0x2000000",           // LARGE_FLAG_BIT (1<<25)
                            "jnz 9f",                            // long lit/len
                            "mov {tmp}, {cnt}",
                            "shr {tmp}, 28",                     // bit_count
                            "jz 9f",                             // bit_count==0 invalid
                            "shrx {read_in}, {read_in}, {tmp}",  // consume (F3, no flags)
                            "sub {ril}, {tmp}",
                            "mov {t2}, {cnt}",
                            "shr {t2}, 26",
                            "and {t2}, 3",                       // t2 = sym_count
                            // last element = (packed >> 8*(count-1)) & 0xFFFF -> tmp
                            "lea {tmp}, [{t2} - 1]",
                            "shl {tmp}, 3",
                            "mov {t3}, {cnt}",
                            "and {t3}, 0x1FFFFFF",               // LARGE_SHORT_SYM_MASK
                            "shrx {tmp}, {t3}, {tmp}",
                            "and {tmp}, 0xFFFF",
                            "cmp {tmp}, 255",
                            "ja 3f",                             // length / EOB
                            // ===== C: speculative 8-byte packed-literal store =====
                            "mov qword ptr [{out_ptr} + {next_out}], {cnt}",
                            "add {next_out}, {t2}",              // advance by sym_count
                            "jmp 2b",
                            // ===== length / EOB path (cnt=entry t2=count tmp=last) ===
                            "3:",
                            "mov qword ptr [{out_ptr} + {next_out}], {cnt}",
                            "lea {t3}, [{t2} - 1]",
                            "add {next_out}, {t3}",              // emit leading literals
                            "cmp {tmp}, 256",
                            "mov {ret}, 0",                      // EOB
                            "je 9f",
                            "cmp {tmp}, 512",                    // MAX_LIT_LEN_SYM
                            "mov {ret}, 4",                      // invalid len sym
                            "ja 9f",
                            "sub {tmp}, 254",                    // tmp = length (kept)
                            // ===== F2: speculative DIST gather in asm =====
                            "mov {t2}, qword ptr [{ctx} + 8]",   // dist_tbl
                            "mov {t3}, {read_in}",
                            "and {t3}, 0x3FF",
                            "movzx {t3:e}, word ptr [{t2} + {t3}*2]",
                            // speculative ret = (length<<3)|3 for the long-dist exit
                            "mov {ret}, {tmp}",
                            "shl {ret}, 3",
                            "or {ret}, 3",
                            "test {t3:e}, 0x400",                // SMALL_FLAG_BIT (1<<10)
                            "jnz 9f",                            // long dist
                            "mov {cnt}, {t3}",
                            "shr {cnt}, 11",                     // dbits (SMALL_SHORT_CODE_LEN)
                            "shrx {read_in}, {read_in}, {cnt}",
                            "sub {ril}, {cnt}",
                            "and {t3}, 0x1F",                    // dsym = entry & DIST_SYM_MASK
                            "cmp {t3}, 30",
                            "mov {ret}, 4",                      // invalid dsym
                            "jae 9f",
                            "mov {t2}, qword ptr [{ctx} + 24]",  // dist_extra
                            "movzx {cnt}, byte ptr [{t2} + {t3}]", // extra
                            "mov {t2}, qword ptr [{ctx} + 16]",  // dist_base
                            "movzx {t2}, word ptr [{t2} + {t3}*2]", // dbase
                            "bzhi {t3}, {read_in}, {cnt}",       // v = read_in & ((1<<extra)-1)
                            "shrx {read_in}, {read_in}, {cnt}",  // consume extra
                            "sub {ril}, {cnt}",
                            "add {t2}, {t3}",                    // distance = dbase + v
                            "cmp {t2}, {next_out}",
                            "mov {ret}, 4",                      // invalid distance
                            "ja 9f",
                            // ===== D: overlap-doubling back-ref copy in asm =====
                            // t3=dst src=cnt temp=t2 countdown=tmp(length)
                            "lea {t3}, [{out_ptr} + {next_out}]", // dst
                            "mov {cnt}, {t3}",
                            "sub {cnt}, {t2}",                   // src = dst - distance
                            "add {next_out}, {tmp}",             // advance next_out by length
                            "cmp {t2}, {tmp}",                   // distance vs length
                            "jb 5f",                             // overlap -> byte copy
                            // non-overlap: igzip-D AVX2 32-byte MOVDQU copy
                            // (overshoot up to 31B licensed by out_slop). Faithful
                            // to VAR_VI's avx_backref_copy non-overlap path, in asm.
                            "4:",
                            "vmovdqu ymm0, ymmword ptr [{cnt}]",
                            "vmovdqu ymmword ptr [{t3}], ymm0",
                            "add {cnt}, 32",
                            "add {t3}, 32",
                            "sub {tmp}, 32",
                            "ja 4b",
                            "vzeroupper",
                            "jmp 2b",
                            // overlap / RLE forward byte copy (self-replicating)
                            "5:",
                            "movzx {t2:e}, byte ptr [{cnt}]",
                            "mov byte ptr [{t3}], {t2:l}",
                            "inc {cnt}",
                            "inc {t3}",
                            "dec {tmp}",
                            "jnz 5b",
                            ...
                            t2 = out(reg) _,
                            t3 = out(reg) _,
                            out("ymm0") _,                       // D AVX copy scratch
                            options(nostack),
                        );
                    }

                    // Write loop-carried state back into `bits` + out_pos. On EVERY
                    // exit the bit cursor sits before a FRESH un-consumed symbol
                    // (long lit/len, boundary), OR right after the lit/len code with
                    // leading literals already emitted (long dist: Rust resolves the
                    // dist+copy), OR at the next block header (EOB).
                    bits.bitbuf = read_in;
                    bits.bitsleft = ril as u32;
                    bits.pos = next_in_pos as usize;
                    out_pos = next_out as usize;
                }
                if cov {
                    asm_bytes += (out_pos - out_pos_before) as u64;
                }

                match ret & 7 {
                    0 => {
                        // EOB: this block done; outer loop reads the next header.
                        break 'asm_reentry;
                    }
                    3 => {
                        // Long DIST code: leading literals already emitted in asm,
                        // lit/len consumed, dist NOT consumed. Resolve the back-ref
                        // in Rust (full dist path), then re-enter the asm.
                        let length = (ret >> 3) as usize;
                        let tb = out_pos;
                        if length != 0
                            && !emit_one_backref(
                                &dist,
                    ...
                    1 => {
                        // Long lit/len code: bits NOT consumed. Decode exactly one
                        // symbol in Rust (validated long path), then re-enter.
                        let tb = out_pos;
                        let one = decode_one_symbol(
                            &litlen,
                            &dist,
                        ...
                        match one {
                            ...
                            OneSym::Eob => break 'asm_reentry,
                            OneSym::Bail => return out[base..out_pos.min(target_end)].to_vec(),
                        }
                    }
                    2 => {
                        // Boundary (out/in slop): finish the remainder with the
                        // bounds-checked careful tail, then leave the asm.
                        let tb = out_pos;
                        let ok = run_careful_tail(
                            &litlen,
                            &dist,
                    ...
                    _ => {
                        // ret&7 == 4: invalid (bad length/dist symbol).
                        return out[base..out_pos.min(target_end)].to_vec();
                    }
                }
            } // 'asm_reentry
        ...
        if cov {
            let total = asm_bytes + tail_bytes;
            eprintln!(
                "VIII_COVERAGE start_bit={start_bit} asm_bytes={asm_bytes} tail_bytes={tail_bytes} \
                 asm_frac={:.4} reentries={reentries}",
                asm_bytes as f64 / total.max(1) as f64
            );
        }
    ...
    const VARIANTS: [(&str, DecodeFn); NV] = [
        ...
        // VAR_VIII = the FULL-KERNEL inline-asm transliteration of igzip's
        // `loop_block`: the back-edge (`jmp 2b`) stays INSIDE the asm with bit-state
        // + in/out pointers + lit/len short-table base PINNED in registers across
        // it. F2 (speculative dist gather) + D (overlap copy) run IN asm; Rust is
        // entered only at long lit/len, long dist, EOB, or boundary. The construct
        // DIS-1 (per-symbol re-entry) does NOT bind. PRE-REGISTERED GATE: PASS iff
        // byte-exact AND asm_frac >= 0.97 AND (VIII)/(III isal) >= 0.85 AND
        // materially > (VI). KILL/PLATEAU iff (VIII) ≈ (VI).
        ("VAR_VIII_fullkernel", decode_var_viii),
    ];
