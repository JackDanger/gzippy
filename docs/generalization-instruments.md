...
# hand ONE cliff coordinate to the next instrument
./target/release/examples/surface_probe --only e8_p8192_long_a256_r0 \
  --keep /tmp/pts --out /tmp/one.tsv
cargo run --release --example divergence_accounting -- --level 1 \
  /tmp/pts/e8_p8192_long_a256_r0
```

`--only` restricts the walk to one declared point and `--keep` writes the
generated bytes to a directory instead of deleting them — a cliff is only useful
if its coordinates can be handed to the instrument that names the mechanism.
