...
| `GZIPPY_ISAL_PURE_BULK` | Route windowed-decode through `decode_chunk_pure_bulk_impl` (ISA-L-LUT bulk decoder from `isal_lut_bulk::decode_block`) instead of `IsalInflateWrapper`. Only available with `cfg(pure_inflate_decode)`. Default OFF pending neurotic confirmation. | PRODUCTION-AFFECTING (inner decoder) |
...
| `GZIPPY_EAGER_POSTPROC` | During consumer stalls, submit `apply_window` for prefetched successor chunks whose predecessor window is already published (port of rapidgzip `queuePrefetchedChunkPostProcessing`). Probe for the eager post-processing lever; off by default. | PRODUCTION-AFFECTING (scheduling) |
