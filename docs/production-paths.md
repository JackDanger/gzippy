...
| `GZIPPY_RESOLVE_AHEAD` | Worker-side `apply_window` at exact `max_acceptable_start_bit` when `WindowMap::contains` that key (vendor post-process). Default **OFF**; `=1` enables after ship-gate. | PRODUCTION-AFFECTING (scheduling) |
