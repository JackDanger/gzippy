...
- **Ours**: YES — `matchfinder/hc.rs` is a port with identical geometry (`HC_HASH3_ORDER 15`, `HC_HASH4_ORDER 16`, `hc.rs:73-98`), same multiplier, same prefilter (`hc.rs:530-549`), same prefetch (`hc.rs:315-330`). Two FALSIFY records live in the file: chain-node prefetch (net loss, `hc.rs:388-403`) and prefilter operand hoisting (Dr up, `hc.rs:499-526`). ⛔ The old claim here — "the remaining gap vs libdeflate is register discipline, not structure — vendor-structure-comparison §4" — is VOID: §4's per-component SPLIT is void (missing 22.7% of our arm vs 7.7% of theirs, the `-N` callgrind-parser defect). Re-measured at L2/T1 with a checked parse (`docs/board/l2-component-map.md`): **our matchfinder is 2.2% CHEAPER than libdeflate's and contributes −12.1% to the excess.** There is no matchfinder gap to close here; the excess is the parse loop (+48.7%, 81.8% of it). Three prior levers aimed at this component and died.
...
- **Ours**: PARTIALLY — scalar u64 XOR + trailing_zeros only (`matchfinder/common.rs:97-133`); **no SIMD compare anywhere in the level engine** (agent-verified). Caveat before borrowing: ⛔ the old §4-based caveat ("our matchfinder gap is register pressure") is VOID — re-measured, our matchfinder is 2.2% CHEAPER than libdeflate's at L2/T1 and contributes −12.1% to the excess (`docs/board/l2-component-map.md`). A SIMD compare would be speeding up the one component we already win. wall-delta ≠ instruction-delta still holds, and harder than before: at L9 we execute 26% MORE matchfinder instructions than libdeflate and WIN that wall cell by 12.3%.
