...
| `src/compress/deflate/anatomy_counters.rs` | The one live campaign instrument in `src/` (Cargo feature `anatomy-counters`, OFF by default; `fulcrum anatomy --counters-from-stderr` ingests it). Gives EXACT work VOLUME, never time. **Known bug (2026-07-25): double-counts `fast_probe_outcome_*` and `fast_positions_processed` by exactly the hash3-rescue count — other counters reconcile to raw_len.** The old `{perturb,phase_timing,storeprobe}.rs` row here was STALE: those modules were deleted in `0e638d81`. **RETRACTED 2026-07-26 by execution (Gate 5a):** an earlier revision of this row claimed `trace`, `counters` and `libdeflate` were "dead flags with ZERO cfg consumers". That was a GREP — i.e. READING — and it is FALSE. Executed instead: build reproducibility control PASSES (two default builds byte-identical, `140ece96…`), and each feature yields a DIFFERENT binary (`trace` a3dd4e0a…, `counters` ee6fc491…, `libdeflate` dea2d574…). They are NOT inert. OPEN QUESTION for Fulcrum, not for a code read: what do they change, and does any of them carry a wall tax (Gate 4b)? |
