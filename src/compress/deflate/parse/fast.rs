    ...
    impl L1Tune {
        fn from_env() -> Self {
            L1Tune {
                ...
                // 2026-07-22 ship decision: the composed HASH3-GATE config
                // (bits=15/max_dist=32768/policy=miss-only/threshold=50/
                // sparse-insert/initial-inactive) is now the DEFAULT L1
                // behavior (see `super::L1_HASH3_BITS` et al. and
                // `super::Hash3Cfg::shipped`) — a feature-on build with NO
                // env override reproduces that shipped default exactly, per
...
/// SHIP DECISION (2026-07-22): the composed HASH3-GATE config — the
/// HASH3-PROBE lever (a genuine 3-byte-key `head3` table making length-3
/// matches visible to the L1 fast path) gated by the CONTENT-ADAPTIVE CHAIN
/// MATCHING lever's zero-cost literal-fraction detector — is now the
/// DEFAULT `Strategy::Fast` (L1) behavior. These are compile-time consts,
/// NOT the `l1-tune` runtime-env-var machinery above (per the "no env vars
/// in the production path" rule): the default (non-`l1-tune`) build always
/// runs this composed lever, unconditionally, for every L1 call. `l1-tune`
/// builds keep the full runtime-tunable `tune::L1Tune` struct for further
/// search — its `hash3_*` fields now DEFAULT to these exact values (see
/// `tune::L1Tune::from_env`), so a feature-on build with no env override
/// reproduces shipped behavior exactly.
///
/// MEASURED (commit `7f735755`, 2026-07-22, size sweep via
/// `examples/l1_search.rs`'s `breadth` subcommand over the real 19-file
/// `~/www/gzippy-bench/corpus` set, cross-arch wall N=21 on Apple M1 Pro +
/// AMD EPYC 7282 + Intel i7-13700T): reaches the mission's ideal on this
/// breadth set — ALL FOUR ungated-hash3's pigz-1 flips survive
/// (`armexe.elf`, `data.parquet`, `tool.bin`, `winexe.exe`, LOSS -> WIN),
/// the ungated lever's one WIN -> LOSS regression (`access.log`) is gone,
/// and two files that crossed the strict `ld1*1.05` gate under the ungated
/// lever (`markup.xml`, `minjs.min.js`) no longer do. See
/// [`tune::L1Tune::hash3_gated`]'s doc comment (still present, used only by
/// the dev search harness) for the full measured story, per-cell numbers,
/// and the honest remaining cost (`data.json` +1.2%, `weights.safetensors`
/// +0.35%, `minjs.min.js` +0.35% even after the gate fix — real, small,
/// doesn't cross any gate).
///
/// `threshold=50` is a real 47-51 robust window (not a knife-edge);
...
/// sub-0.1%-either-direction size difference; `hash3_gate_initial_active=
/// false` (start silent until the first block's literal fraction is known)
/// is what makes several large text-like breadth files come out
...
/// histogram — no extra scan). Measured robust across a 47-51 window; 50 is
/// the chosen point.
pub(super) const L1_HASH3_GATE_LIT_THRESHOLD_PCT: u32 = 50;
...
/// Start each `run()` call with the gate INACTIVE (probe-silent) until the
/// first block's literal-fraction signal is known. Measured: this is what
/// makes several large text-like breadth files come out byte-IDENTICAL to
/// the pre-lever baseline (they never trip the threshold on any block,
/// including — with this const `false` — the first one).
pub(super) const L1_HASH3_GATE_INITIAL_ACTIVE: bool = false;
...
pub(super) fn run<const ACCEL: bool>(
    ...
    // 2026-07-22 ship decision — see [`Hash3Cfg`]'s doc comment), or
