    ...
    if let Some((length, dist)) = accepted {
        ...
        while nh < insert_end {
            ...
            if INTERLEAVED {
                ...
                // SHIFT THE WHOLE BUCKET on the T1 REACH route — see
                // `apply_l1_match_reach_t1_knobs`. `REACH` is a const generic
                // so this compiles away on the T>1 and legacy T1 routes.
...
pub(super) fn run<const ACCEL: bool, const REACH: bool, const INTERLEAVED: bool>(
    ...
    debug_assert!(
        !(REACH && INTERLEAVED),
        "T1 REACH and T>1 interleaved bucket are separate monomorphizations"
    );
