...
// Dead in a default build while L1 routes to `super::ht_fast`. Still live under
// `l1-tune`. Retained rather than deleted until the ht path has cleared a WALL leg —
// the prior attempt at this class died on wall, so the fallback stays compiled.
#[allow(dead_code)]
