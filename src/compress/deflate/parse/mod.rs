...
/// has ever satisfied.
///
/// MEASURED 2026-08-01: **+2.36% wall** at L6 T1. The lever is ALIVE.
///
/// `fulcrum ab paired --mode compress`, A = this lever (hoisted scratch),
/// B = vanilla `58f3968b`, tool.bin, /dev/null both arms:
///
///     verdict RESOLVED-a-slower   ratio 1.0236   logratio_ci95 [0.0188, 0.0280]
///     a_median 557.050 ms   b_median 542.759 ms   n=21   sign 20/21
///     aa_bias 0.0018        sha_ok true          roundtrip_ok true
///     size 20,815,473 vs 20,825,308   size_ratio 0.999528   size_stable true
///
/// WHERE THAT LANDS. Clause 5's budget is ~1.0% of self-slowdown at T1 and ~2.6%
/// at T4 (see `project_clause5_is_the_binding_constraint`). 2.36% is OVER at T1 and
/// UNDER at T4 — the same envelope as PR #227's `max_search_depth * 4` (+2.3%), the
/// only size lever of this campaign to reach a gate. So this is a T>1 candidate,
/// which is also where 125 of the 200 failing cells live. Adjudicate with
/// `fulcrum try <ref> --threads 1,4`; a paired win is not a ship decision.
///
/// ⛔ RETRACTED, AND WHY IT MATTERS. This comment previously carried FOUR stacked
/// FALSIFY notes claiming this function was 1.8x-6.4x slower, plus a fifth
/// indicting `anatomy-wall` for a "6x instrumentation tax" and voiding
/// `parse_match_ns = 89.1%`. ALL OF IT WAS ONE ARTIFACT of a hand-rolled harness
/// whose two arms were not comparable:
///
///     ours_cmd  = [OURS,  "-{level}", "-p1", "-c", PATH]
///     rival_cmd = [RIVAL, "-{level}",        "-c", PATH]   <- NO -p1
///
/// Written for libdeflate, which has no `-p` flag. Putting one of OUR binaries in
/// the rival slot ran it MULTITHREADED against a `-p1` arm on a 10-core box. The
/// same binary against ITSELF scored 6.10x. Nothing was ever measured.
///
/// `fulcrum ab paired` is documented as carrying a MANDATORY A/A CERTIFICATE and
/// VOIDing on `aa_bias`. Its certificate here reads 0.0018. Hard stop #6 — "never
/// hand-roll a measurement, check Fulcrum's command list first" — names this exact
/// failure, and the tool has the exact guard for it. Full retraction: ef023325.
///
