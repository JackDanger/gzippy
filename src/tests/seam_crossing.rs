...
mod tests {
    ...
    // ── (b) Read-before-write under POISONED reserve (advisor item b) ──────────
    //
    // The clean-tail copy-free path writes u8 directly into the reserved
    // (uninitialized) spare; a read-before-write bug reads garbage that a normal
    // differential catches only PROBABILISTICALLY (zeroed pages can read as the
    // correct byte). This test runs the fold path with the reserved spare
    // POISONED to 0xCD (`GZIPPY_POISON_RESERVE`), so any read-before-write
    // deterministically corrupts the output → byte-equality fails every run.
    //
    // The poison is gated to this thread: the env var is set, then the decode
    // runs on a FRESH thread so the poison's per-thread cache reads it as ON,
    // and no timing-sensitive test (which runs on other threads with the cache
    // already OFF) is perturbed by the poison memset. A clean PASS here proves
    // the production fold path writes-before-read across the flip seam AND across
    // the >16 MiB clamp regrow boundary.
    #[test]
    fn seam_poisoned_reserve_no_read_before_write() {
        // Cover both clamp regimes: a sub-16-MiB tail and an over-16-MiB tail
        // (which regrows). Run under the poison on a dedicated thread.
        std::env::set_var("GZIPPY_POISON_RESERVE", "1");
        let handle = std::thread::Builder::new()
            .name("seam-poison".into())
            .stack_size(8 * 1024 * 1024)
            .spawn(|| {
                for (raw, period, min_clean, label) in [
                    (
                        20 * 1024 * 1024usize,
                        36 * 1024 + 1,
                        15 * 1024 * 1024usize,
                    ...
                    let payload = periodic_max_distance(raw, period);
                    drive_single_large_chunk(&payload, 6, label, min_clean);
                }
            })
            .expect("spawn poison thread");
        let result = handle.join();
        std::env::remove_var("GZIPPY_POISON_RESERVE");
        result
            .expect("poisoned-reserve fold decode read uninitialized tail (read-before-write bug)");
    }
}
