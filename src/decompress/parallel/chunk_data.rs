...
impl ChunkData {
    ...
    pub fn finalize_with_deflate(
        ...
        // subchunk reports decoded_size 0, violating the vendor split()
        // sum invariant (ChunkData.hpp:736-744), making
        // `merge_small_trailing_subchunk_if_needed` treat every trailing
        // subchunk as below the merge threshold, and feeding zero sizes
        // into the BlockMap accounting.
        let total_decoded = self.decoded_size();
        if let Some(last) = self.subchunks.last_mut() {
