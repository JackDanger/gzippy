...
fn phase1c_resolve_consistency(
    ...
    // Once chunk i is stable (chunk[i].is_some() && start_bits[i] == pred_end
    // or i == 0), the scan never needs to revisit i. Without this cursor a
    // pathological all-None phase-1a run would do O(N) scan work per wave for
    // O(N) waves = O(N²) total — unacceptable on large files.
    //
    ...
    loop {
        ...
        for i in scan_start..num_chunks - 1 {
            ...
            if !needs {
                // Both chunk[i] and chunk[i+1] are stably resolved; the scan
                // never needs to revisit i again. Advance the cursor past i so
                // the next wave skips already-stable prefixes in O(1).
                if new_scan_start == i {
                    new_scan_start = i + 1;
                }
                continue;
