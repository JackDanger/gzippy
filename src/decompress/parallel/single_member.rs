...
pub fn decompress_parallel<W: Write>(
    ...
    // for fd-backed sinks (File/Stdout); `None` keeps the buffered
    // `write_all` path (MmapWriter / pipe-unbeneficial / test writers).
    // The fd is computed at the `io.rs` boundary where the concrete
    // sink type is known. See `chunk_fetcher::fd_vectored_write`.
    out_fd: Option<i32>,
    num_threads: usize,
