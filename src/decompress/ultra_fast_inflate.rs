...
use crate::decompress::two_level_table::{decode_symbol, FastBits, TwoLevelTable};
...
fn decode_huffman_block(
    ...
    loop {
        ...
        crate::decompress::simd_copy::lz77_copy_fast(output, distance, length);
    }
    ...
    loop {
        ...
        match entry.distance {
            ...
            distance => {
                ...
                crate::decompress::simd_copy::lz77_copy_fast(output, dist, length);
            }
        }
    }
...
pub fn inflate_gzip_preallocated(input: &[u8], output: &mut Vec<u8>) -> io::Result<usize> {
    ...
    let written = crate::decompress::bgzf::inflate_into_pub(deflate_data, out_slice)?;
...
fn profile_ultra_fast_components() {
    ...
    // Parse gzip header
    let header_size = crate::decompress::parallel::single_member::skip_gzip_header(&data).unwrap();
...
fn profile_lz77_copy() {
    ...
    let start = std::time::Instant::now();
    for _ in 0..100_000 {
        crate::decompress::simd_copy::lz77_copy_fast(&mut buffer, 1, 32); // RLE
        crate::decompress::simd_copy::lz77_copy_fast(&mut buffer, 4, 32); // Pattern
        crate::decompress::simd_copy::lz77_copy_fast(&mut buffer, 64, 128); // Far copy
    }
    ...
    let start = std::time::Instant::now();
    for _ in 0..10_000 {
        crate::decompress::simd_copy::lz77_copy_fast(&mut buffer, 1000, 1000);
    }
