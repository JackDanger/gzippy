...
use crate::backends::isal::IsalInflater;
...
impl UltraInflate {
    ...
    fn decompress_sequential<W: Write>(&self, data: &[u8], writer: &mut W) -> io::Result<u64> {
        ...
        match crate::decompress::bgzf::inflate_into_pub(data, &mut output) {
            Ok(actual) => {
...
fn decode_chunk(
    ...
    if let Ok(actual) = crate::decompress::bgzf::inflate_into_pub(data, &mut output) {
            ...
            scope.spawn(move || {
                loop {
                    ...
                    if deflate_start < deflate_end {
                        ...
                        let _ = crate::decompress::bgzf::inflate_into_pub(deflate_data, out_slice);
                    }
                }
            });
        }
