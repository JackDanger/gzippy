...
CSV = sys.argv[1] if len(sys.argv) > 1 else "/dev/shm/ixv_conv_perf.csv"
...
def block(test, aArm, bArm, aLbl, bLbl, winlabel):
    ...
    for c in ["silesia", "monorepo", "nasa"]:
        ...
        print(f"[{c}] out={sz/1e6:.0f}MB  n={len(I[(test[0],c,aArm)])}")
        print(f"  A {aLbl:24s} instr/B={Ai/sz:7.3f} cyc/B={Ac/sz:6.3f} (spread {Asi:.2f}%)")
        print(f"  B {bLbl:24s} instr/B={Bi/sz:7.3f} cyc/B={Bc/sz:6.3f} (spread {Bsi:.2f}%)")
        print(f"  >> instr B/A={ri:.4f}x  Δ={eff:+.2f}% vs spread {sp:.2f}% -> {v}")
        print(f"  >> cyc   B/A={rc:.4f}x [NOISY: unfrozen LXC — context only]  selftest A2/A={selft:.5f}")
    print()

block(("DB", "DOUBLE-BUILD DROP: converged (@cffa61ee) vs hybrid (@c4c3cc97), BOTH asm-off + FLAT_CLEAN=1"),
      "A_converged", "B_hybrid", "converged(cffa61ee)", "hybrid(c4c3cc97)", "CONVERGED WINS (double-build dropped)")
...
block(("D3", "engine A (flat pure-Rust, asm-OFF) vs run_contig (x86 BMI2 ASM, asm-ON)"),
