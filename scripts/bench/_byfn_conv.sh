#!/bin/bash
# By-region decomposition on the CONVERGED HEAD (fa05032a==c8bb5d78 kernel) vs igzip.
# instructions+cycles, T1 pinned cpu4. ONE perf instance at a time.
# PHASE 1: perf stat N=7 interleaved (whole-decode totals).
# PHASE 2: perf record -g --call-graph=dwarf + perf report --no-children --sort symbol
#          (self-time per symbol) per event, percent-limit 0.3.
set -u
GZ=/dev/shm/gzconv-target/release/gzippy
IGZIP=/usr/bin/igzip
PIN=4
N=7
DD=/dev/shm/perfconv
...
echo "### PHASE 2: perf record -g --call-graph=dwarf per-symbol self-time split (sequential)"
record_report() { # tag tool gzfile
  local tag=$1 tool=$2 f=$3
  if [ "$tool" = gz ]; then
    perf record -e cpu_core/instructions/,cpu_core/cycles/ -g --call-graph=dwarf -o "$DD/$tag.data" -- \
  ...
  # self-time per symbol (--no-children), flat
  perf report -i "$DD/$tag.data" --stdio --no-children --percent-limit 0.3 -e cpu_core/instructions/ --sort symbol 2>/dev/null > "$DD/$tag.instr.rpt"
  perf report -i "$DD/$tag.data" --stdio --no-children --percent-limit 0.3 -e cpu_core/cycles/ --sort symbol 2>/dev/null > "$DD/$tag.cyc.rpt"
}
