...
"${NEUROTIC[@]}" "scp -o StrictHostKeyChecking=accept-new $BENCH_HOST_DIR/guest_engine_isolation.sh root@REDACTED_IP:$BENCH_HOST_DIR/"
...
stage_one() { # <local-path> <staged-name>
  ...
  "${NEUROTIC[@]}" "scp -o StrictHostKeyChecking=accept-new $BENCH_HOST_DIR/_stage_$2 root@REDACTED_IP:$STAGE_GUEST/$2"
}
