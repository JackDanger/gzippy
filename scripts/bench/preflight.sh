...
#   (b) BOX IDENTITY  — solvency(AMD,REDACTED_IP) vs trainer/lxc199(Intel,
#                       REDACTED_IP) were confused for whole rounds. The intended
...
box_row() {
  case "$1" in
    solvency)      echo "solvency x86_64 REDACTED_IP amd-boost0-llamastop" ;;
    trainer)       echo "trainer  x86_64 REDACTED_IP neurotic-benchlock"  ;;
    lxc199)        echo "trainer  x86_64 REDACTED_IP neurotic-benchlock"  ;;
...
RIG MANIFEST (enforced by preflight.sh) — see memory reference_rig_manifest.md
  solvency  = AMD EPYC 7282 Zen2, ssh jackdanger@REDACTED_IP (physical host).
  ...
  trainer   = Intel i7-13700T LXC 199 on neurotic, ssh -J neurotic root@REDACTED_IP.
