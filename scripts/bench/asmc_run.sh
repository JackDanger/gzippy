#!/bin/bash
# Frozen bracket for the rung-(c) A/B (asma_run.sh pattern).
set -u
...
echo "== T1 silesia n=$N =="
$SSH_GUEST "bash /root/asmc_ab.sh silesia 1 0 $N"
echo "== T8 model n=$N =="
$SSH_GUEST "bash /root/asmc_ab.sh model 8 0,2,4,6,8,10,12,14 $N"
hostlock_release
