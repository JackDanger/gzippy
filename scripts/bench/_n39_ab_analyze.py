...
OUT=sys.argv[1] if len(sys.argv)>1 else "/dev/shm/n39_ab"
B={}
...
print("=== NIGHT39 production A/B (cyc/B) — clean-path speculative-guard gating ===")
for c in ("silesia","monorepo"):
    if (c,"new") not in data: continue
    print(f"\n--- {c} (bytes={B[c]}) ---")
    for tool in ("new","base","new2","igzip"):
        ...
        llcb=st.median([llc/B[c]*1e6 for (_,_,_,llc) in vals])
        ghz=[cyc/(tc*1e6) for (_,cyc,tc,_) in vals]
        ghzspread=(max(ghz)-min(ghz))/st.median(ghz)*100
        print(f"  {tool:5s}: cyc/B={cb:.4f} instr/B={ib:.4f} IPC={ipc:.3f} LLCmiss/MB={llcb:.2f} GHz={st.median(ghz):.3f} (spread {ghzspread:.2f}%)")
    n,ma,mb,m,lo,hi=paired(c,"new2","new")
    sig="OK(brackets 0)" if (lo<0<hi) else "!! SELF-TEST FAIL (box noisy)"
    print(f"  [Gate-0 self-test] new2-new cyc/B Δ: {m:+.4f} [{lo:+.4f},{hi:+.4f}] N={n} -> {sig}")
    n,ma,mb,m,lo,hi=paired(c,"new","base")
    sig="CI-DISJOINT(win)" if hi<0 else ("CI-DISJOINT(regress)" if lo>0 else "TIE")
    print(f"  [VERDICT] new-base cyc/B Δ: {m:+.4f} [{lo:+.4f},{hi:+.4f}] N={n} -> {sig}")
    n,ma,mb,m,lo,hi=paired(c,"new","igzip")
    print(f"  new-igzip remaining gap:  {m:+.4f} [{lo:+.4f},{hi:+.4f}]")
