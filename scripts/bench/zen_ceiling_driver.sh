#!/usr/bin/env bash
# ZEN ceiling AMD driver: pause llama (GUARANTEED resume) + freeze + measure + restore.
set -u
OUT=/dev/shm/zen-ceil-out
mkdir -p "$OUT"
LOG="$OUT/driver.log"; exec > "$LOG" 2>&1
PIDFILE="$OUT/llama.pids"
WATCHDOG_S="${WATCHDOG_S:-1800}"
...
setsid bash -c "sleep $WATCHDOG_S; while read -r p; do [ -n \"\$p\" ] && kill -CONT \"\$p\" 2>/dev/null; done < '$PIDFILE'; echo '$ORIG_GOV' | tee /sys/devices/system/cpu/cpu*/cpufreq/scaling_governor >/dev/null 2>&1; echo '$ORIG_BOOST' > /sys/devices/system/cpu/cpufreq/boost 2>/dev/null; echo watchdog-fired-$(date -u +%FT%TZ) >> '$OUT/watchdog.log'" >/dev/null 2>&1 < /dev/null &
WD=$!
...
CORPUS_DIR=/root OUT="$OUT" N="${N:-11}" \
CELLS="${CELLS:-silesia:4 squishy:4 silesia:2 monorepo:2 silesia:8 nasa:4}" \
