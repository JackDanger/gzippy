...
| **F** | Window-absent bootstrap (`d_w`) | `worker.bootstrap` **~2121 ms** busy; +30% wall @ +100% bootstrap slow-inject | `deflate_block` ring prefetch (mirror resumable); causal **confirmed** (+12% wall @ +100% inject, N=3) | Open (prefetch **TIE** wall; −25 ms bootstrap busy) |
