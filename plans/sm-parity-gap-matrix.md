...
| **H** | Partition-seed key mismatch | 37/38 WA = KEY-MISMATCH | Worker `get_at_worker` + `get_handoff_in_partition` (speculative merge); `GZIPPY_SPEC_PRED_CLEAN` oracle (default off) | **TIE** (2026-06-05): counters `handoff@stop=0`, `boundary@seed=0`; windows not at seed at prefetch start — see `docs/structural-gap-rapidgzip.md` §2 |
