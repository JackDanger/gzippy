...
to STEP-0's frozen binary). **Box:** Intel i7-13700T LXC (guest REDACTED_IP) **FROZEN** via
`REDACTED_IP:/root/bench-lock.sh acquire 1800` (`BENCH_LOCK=quiet runnable_avg=1.50`,
...
## REPRODUCE
```
ssh root@REDACTED_IP /root/bench-lock.sh acquire 1800   # require BENCH_LOCK=quiet
...
ssh -J REDACTED_IP root@REDACTED_IP 'GZ=/dev/shm/gzrg-target/release/gzippy \
