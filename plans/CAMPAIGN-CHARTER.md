...
## CURRENT STATE (2026-06-07, owner turn, branch reimplement-isa-l) — **PORT (ii) LANDED: rg's view-based applyWindow (merge-removal). Byte-exact. T8 wall MOVED ~+12% (0.65×→0.73× of rg), advisor-UPHELD.** The redundant full-output memcpy is gone. NEXT = port (i) rg's multi-cached u16 marker loop (decodeBlock 1.69×) — the larger remaining gap. **SUPERVISOR GATE — step 1 measured + committed; step 2 (marker loop) NOT started.**
