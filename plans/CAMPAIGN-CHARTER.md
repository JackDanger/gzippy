...
## CURRENT STATE (2026-06-08, owner turn, branch reimplement-isa-l, HEAD 8d4f20f7+) — **CACHE-RESIDENCY LITERAL ARCHITECTURE MET (the two SCOPED items closed, byte-exact, re-measured). (1) DIST CACHE SHRUNK 128 KiB→8 KiB/thread (commit 8d4f20f7): ported vendor's bounded-LUT `HuffmanCodingShortBitsCached` (huffman/HuffmanCodingShortBitsCached.hpp) for the production distance path — LUT_BITS_COUNT=12, Symbol=u8 ⇒ (u8,u8)*2^12 = 8 KiB (was [(u8,u16);1<<15] = 128 KiB, itself 2× the vendor's own 64 KiB Symbol=uint8_t distance cache, deflate.hpp:336/668), decode_long fallback for >12-bit codes (new file src/decompress/parallel/huffman_short_bits_cached.rs). PER-THREAD 278.8→158.8 KiB (target ~150 → MET). (2) SegmentedU8 DecodedData: BOTH original-port levers (data:SegmentedU8 + in-place narrowed_len) ALREADY in tree; the only un-landed piece (physical segment-list backing) was DELIBERATELY/FAITHFULLY superseded by e8c03110 (contiguous DecodedData, DecodedData.hpp:278-289) — re-landing would REGRESS the ContigFold copy-free tooth (decode_clean_into_contig base+cap write) the verdict forbids AND diverge from vendor; REJECTED with mechanism (rule #7b). (3) output_ring 128 KiB KEPT (vendor m_window16 floor). RE-MEASURED (frozen guest 10.30.0.199, bin_sha e32829a8, sha==028bd002…): RSS-vs-T +204.5% (was +216%); per-thread 158.8 KiB; MPKI LLC-miss 0.204 (<rg 0.367), L1d-miss 4.03 (<rg 5.89) — NO regress (improved); ballast control linear 16.0/16.0/15.8 thr/MiB; WALL parity.sh T8 0.899× TIE / T16 0.921× TIE (sha=OK) — NO wall regress. DUAL-SHA both builds: gzippy-native + gzippy-isal (x86 via Rosetta) byte-exact @L1/L6/L9, path=ParallelSM. ADVISOR claude -p (Opus) SPAWNED OK — disproof on dist-cache byte-exactness (5 vectors) → claim SURVIVES (new≡base≡old; only error-timing on truncated streams differs, outside legal-stream scope). HARNESS CAVEAT: rss_vs_t.sh MPKI label prints `-nan` (awk divisor bug with cpu_core/-prefixed counter); RAW counters valid, MPKI computed from them. Verdict: plans/cache-residency-architecture-met.md.**
NEXT (optional, do NOT start without supervisor): fix the rss_vs_t.sh MPKI awk label (cosmetic; raw counters fine); separate per-chunk-type MPKI; the gzippy-native engine 1.0× fork (A-vs-B, low-T engine-bound LOSS) is unchanged and independent of this footprint work (footprint = wall-slack, confirmed TIE).

## SUPERSEDED — PRIOR CURRENT STATE (2026-06-08, owner turn, branch reimplement-isa-l) — **CACHE-RESIDENCY MANDATE MEASURED (the never-measured DECISIVE clause). Re-hooked mem_stats to the REAL native per-thread working set (`BOOTSTRAP_BLOCK`/`marker_inflate::Block`, post-fold — the prior hook was DEAD on Engine-C staging and read threads=0); built `scripts/bench/rss_vs_t.sh`(+guest); measured on the frozen quiet guest (16-core RaptorLake, sha-verified, path=ParallelSM). Instrument VALIDATED: threads=16, ballast positive-control recovers the per-thread slope linearly (16 threads/MiB), OFF==identity, 881 lib tests pass. NUMBERS: per-thread working set = 278.8 KiB (TWO 128 KiB structs: output_ring + dist_hc.code_cache; +22.5 KiB lut_litlen; shared tables only 18 KB). RSS-vs-T: gzippy 122.8/322.2/388.1 MB @T1/8/16 vs rapidgzip 67/211/312 (1.83×/1.52×/1.24×); +216% growth T1→T16 (NOT flat). The stale wall-progress 3× reading is pre-fold; post-fold the gap is 1.24–1.52×. MPKI: gzippy LLC-miss 0.205 / L1d-miss 4.55 — actually BELOW rapidgzip (0.379 / 5.77), though gzippy runs 1.48× more instructions. VERDICT: cache-residency mandate NOT fully met on working-set-size/RSS-flatness, but its PURPOSE (low MPKI / hot-in-cache) is ALREADY satisfied — the 278 KiB streams with locality, no measured miss penalty. Cross-checks the banked 5-refutation x86 STOP (footprint=wall-slack). SCOPED for next turn (do NOT start): shrink/share the 128 KiB dist_hc.code_cache (CACHE-LEN 2^15 vs ~30 dist symbols), re-land SegmentedU8 DecodedData for RSS-flatness. NO production change (instrument byte-transparent, scripts measurement-only). Verdict: plans/cache-residency-verdict.md.**

## SUPERSEDED — PRIOR CURRENT STATE (2026-06-08, owner turn, branch reimplement-isa-l, HEAD 19add96c) — **GOAL #2 SHIPPED: gzippy-isal NOW ROUTES THE PRODUCTION CLEAN TAIL THROUGH REAL ISA-L FFI (faithful rapidgzip WITH_ISAL), no longer env-gated. The second implementation the /goal mandates is COMPLETE — gzippy-isal is the at-parity reference: it TIES rapidgzip at T8 and is far closer than native at low-T. Byte-exact on BOTH features; full lib suite + correctness net + hardened differential gate green; advisor UPHELD-W-CAVEATS.**

THIS TURN (HEAD 19add96c): promoted the proven copy-free ISA-L clean-tail oracle from the `GZIPPY_ISAL_ENGINE_ORACLE` env knob to the PRODUCTION routing on the `isal_clean_tail` (gzippy-isal) build. `isal_engine_oracle_enabled()` now defaults to `cfg!(isal_clean_tail)` (env 1/0 still force on/off — 1 = native-build measurement oracle, 0 = pure-Rust for the differential gate's LEFT); `finish_decode_chunk_impl` gained an `allow_isal` param (true in production). The clean tail (post-32KiB-flip bulk via FlipToClean + window-seeded chunks) decodes through `decompress_deflate_from_bit_into` exactly like rapidgzip's `finishDecodeChunkWithInexactOffset<IsalInflateWrapper>` (GzipChunk.hpp:440-444 + :520-526); the <=32KiB markered prefix stays the pure-Rust marker engine (deflate::Block) — faithful. gzippy-native UNCHANGED (isal_chunks=0, pure-Rust FOLD, byte-exact).

TWO BYTE-EXACTNESS FIXES the silesia-only oracle never exposed (caught by the btype01-heavy + multi-subchunk routing traps, which the prior oracle never ran):
1. **OVER-DECODE**: ISA-L's END_OF_BLOCK stop does NOT fire on stored/fixed blocks, so a stored-block-heavy chunk recorded ZERO boundaries and the inexact coalesce committed a multi-MiB over-decode past stop_hint (mis-seeded next chunk → "Stored block len/nlen mismatch"). Fix: accept `(end_bit, written)` only when `end_bit <= stop_hint` (genuine BFINAL member-end); else DECLINE to byte-exact pure-Rust (counted in ISAL_ENGINE_ORACLE_FALLBACKS).
2. **SUBCHUNK SPLIT**: the oracle credited the full keep_len before replaying boundaries, hiding per-segment growth so `append_block_boundary_at` never split the ISA-L tail (UNSPLIT_BLOCKS_EMPLACED trap). Fix: credit each [prev_off, boundary) segment via `note_inner_decoded_bytes` BEFORE recording its boundary (vendor GzipChunk.hpp:321). Byte-transparent.

MEASURED (x86_64 ISA-L guest, parity.sh bench-lock, interleaved N=11, sha-verified vs rg 0.16.0):
| T | gzippy-isal | rg | ratio | verdict | native (banked) |
|---|---|---|---|---|---|
| 1 | 1019 ms | 921 ms | **0.904×** | LOSS (tight 1%/1%) | 0.608× |
| 4 | 554 ms | 490 ms | **0.885×** | TIE (rg spread 46% loaded) | 0.755× |
| 8 | 363 ms | 374 ms | **1.030×** | **TIE** | 0.885× |
ISA-L coverage: isal_chunks=14/14 fallbacks=0 at T4/T8; 16/1 at T1 (the 1 = until_exact exact-stop inflate_wrapper chunk, byte-exact pure-Rust net, faithful). Reproduces the TIME-ACCOUNTING ocl_REAL exactly (T1 1015/T4 548/T8 365).

CORRECTNESS: silesia dual-sha == 028bd002…cb410f at T1/T4/T8 on BOTH features; full lib suite green under GZIPPY_POISON_RESERVE=1 (the one non-green was a missing test_data/alice.txt fixture on the guest — env, not logic — passes once synced; the fd_vectored_write reader-death test is a pre-existing load-sensitive flake, orthogonal); hardened isal_tail_parity differential gate 10/10 (until_exact false+true, fields bytes/len/final_bit/crc/boundaries); btype01-heavy + coordinator-boundary + unsplit-subchunk + single-member-routing green.

ADVISOR (synchronous disproof, plans/isal-production-advisor-verdict.md): **UPHELD-WITH-CAVEATS**. All byte-exactness/fallback break attempts FAILED — declines return BEFORE any commit (clean handoff, no half-mutated chunk); the over-decode accept is provably reachable only via BFINAL within ~64 bits of slice_end (true final chunk); incremental note telescopes to exactly keep_len. CAVEAT (honest scoping, non-corrupting): "faithful WITH_ISAL" is faithful *where ISA-L's EOB-stop contract holds* — on a stored/fixed-block-heavy corpus declines become common and gzippy-isal degrades to gzippy-native (byte-exact, but zero ISA-L coverage; the TIE-vs-rg perf claim holds only on all-dynamic data like silesia). Fully closing it needs porting rapidgzip's read_in resync (isal.hpp:392-405) for non-dynamic blocks — OUT of this turn's scope; correctness is unaffected.

NEXT (owed, optional): port the ISA-L non-dynamic read_in resync to extend faithful ISA-L coverage to stored/fixed-block inputs (closes the advisor caveat). The gzippy-native 1.0× engine fork (A-vs-B, prior CURRENT STATE) is unchanged and independent.

## SUPERSEDED — PRIOR CURRENT STATE (2026-06-08, owner turn, branch reimplement-isa-l, HEAD f216c691) — **TOOLING-AUDIT FIXES LANDED + DECISION INPUTS RE-MEASURED (interleaved RATIO, hardened path, advisor UPHELD-W-CAVEATS). The two SUSPECT/ARTIFACT numbers are RESOLVED in DIRECTION: ocl_cf was PESSIMISTIC — with the 64MiB-reserve confound fixed it rises to ≈TIE/≥0.945× rg (not 0.945×). The A-vs-B fork input is now: a low-T engine-bound LOSS to rg DOES exist (native T1 0.608×, T4 0.755×; T1==isolated inner-loop rate ⇒ engine-bound), BUT "a better engine CLOSES it" is NOT yet measurement-clean (ocl_cf-T4 spread 16% admits the null). NO production change (oracle path gated behind GZIPPY_ISAL_ENGINE_ORACLE, OFF==identity; scripts measurement-only; byte-exact preserved).**

THIS TURN (HEAD f216c691):
- **HARDENED the oracle.sh / _oracle_guest.sh path to the parity.sh bar** (commit e4389f05): env-scrub ALLOWLIST, content-fingerprint stale-binary guard (+ oracle.sh --build stamps it), host-freeze HARD-FAIL on a readable thaw (was WARN-only — the source of ocl_cf's 0.945↔0.989 drift), loadavg readback HARD-FAIL >2.0 (ALLOW_LOAD=1 → ratio-only), IN-SCRIPT isal_oracle_fallbacks==0 assert read from the GZIPPY_VERBOSE sidecar (was hand-checked). The host-freeze guard CAUGHT the box thawed (no_turbo=0, watchdog expired) — re-froze via neurotic /root/clock_freeze.sh freeze 3600.
- **FIXED the 64MiB-per-chunk reserve confound** (e4389f05 + f216c691): the ISA-L oracle reserved a flat 64MiB/chunk where production grows incrementally to ~chunk size; now chunk-proportional reserve = (compressed_span × 8).max(4MiB).min(64MiB) ⇒ ~38MiB for a 4.7MiB T8 chunk (genuine reduction, factor-16 still hit the 64MiB cap). Under-reserve is SAFE (FFI returns None → pure-Rust fallback → counter → assert VOIDs loud). Byte-exact (gated).
- **GUEST DISK cleaned**: 90% (3.4G free) → 81% (6.4G free). Removed /root/gzippy/{target,benchmark_data} (non-pin checkout, rebuildable/regenerable), stale /tmp build trees + ~6G of stale decoded-output dumps (tmpfs RAM). The pin /root/gzippy-bench + corpus /root/silesia.gz UNTOUCHED. A fresh isal build (~800M) now fits.
- **QUIET-BOX RE-MEASURE — BLOCKED-on-load for ABSOLUTES; RATIO captured (interleaved, jitter-immune per the audit; sha-OK; fallbacks==0 verified in-script):** the box was Plex-loaded the whole turn (3+ Plex transcoders + ffmpeg, loadavg bouncing 1.6↔11 as new sessions arrived; polled bounded ~25min, never reliably quiet). Plex runs on the HOST (neurotic), outside the LXC + outside clock_freeze's cgroup-freeze reach. So ABSOLUTE walls + the 36/21 split are NOT banked (would manufacture the audit's artifact). RATIO numbers (gzippy-isal bin b3d9435587186f27 ocl_cf; gzippy-native bin 9790e8f3 production):
  | cell | gzippy-native (prod) | ocl_cf (ISA-L ceiling) | rg | note |
  |---|---|---|---|---|
  | T8 | 0.885× (19% spread, TIE) | **≈0.99× / ≥0.945×** (14/0 clean; one run 0.997×@load1.63) | 1.0 | ocl_cf UP from banked 0.945× PESSIMISTIC |
  | T4 | **0.755× LOSS** (3% spread — tight) | ~0.900× (16% spread — loose, admits null) | 1.0 | engine helps but T4 closability NOT clean |
  | T1 | **0.608× LOSS** (4% spread — tight; ==inner-loop 0.55-0.60× ⇒ engine-bound) | VOID (bootstrap chunk window-absent ⇒ no ISA-L ceiling) | 1.0 | engine-bound LOSS, no ceiling |
- **36/21 engine/non-engine split:** NOT re-measured (the absolute split needs a genuinely quiet box; still OWED). Do NOT bank 36/21.
- **ADVISOR (synchronous disproof, plans/oracle-hardening-advisor-verdict.md): UPHELD-WITH-CAVEATS** — hardening+reserve-fix byte-exact and safe (factor-8 under-reserve VOIDs loud not silent; expect occasional VOIDs on compressible chunks); bank ocl_cf as "≈TIE/≥0.945×" not "0.997×"; **the low-T LOSS is real + engine-bound (strong), but "a better engine CLOSES it" is NOT established — ocl_cf-T4's 16% spread admits "engine swap does nothing, residual is non-engine"; T1 ocl_cf is VOID so T1 can't prove closability.** OWED to make option-B clean: a TIGHT (≤5% spread, quiet box) ocl_cf-T4-vs-native-T4-vs-rg.

**SUPERVISOR GATE — DECISION INPUTS for the A-vs-B fork, VALIDATED in direction, with one OWED tightening. (A) ACCEPT ~0.86-0.99× pure-Rust + close the engine chapter, OR (B) authorize a multi-session full-kernel asm. NEW since the audit: ocl_cf is NOT 0.945× — the clean ISA-L-class path ≈TIES rg (the confound was pessimistic), so option B's whole-system UPSIDE at T8 is ~nil (T8 native 0.885× is already ~TIE-by-spread and ocl_cf≈1.0× confirms little engine headroom is wall-visible at T8). The case FOR B rests ENTIRELY on the LOW-T cells: native LOSES at T1 (0.608×) + T4 (0.755×) and T1 is engine-bound — BUT the advisor REFUTED "engine-closable" as not-yet-clean at T4's 16% spread. NEXT OWED before (B) is authorizable: ONE tight (≤5%, genuinely quiet box) ocl_cf-T4 to confirm the engine swap actually closes the T4 gap (vs the residual being non-engine placement/scheduling). If ocl_cf-T4 ties rg with native losing ⇒ B is justified at low-T; if ocl_cf-T4 ≈ native ⇒ the low-T gap is NON-engine and B is NOT justified (close the engine chapter, attack low-T scheduling). The owner could not get a quiet box this turn (active Plex on the host).** GUEST: /root/gzippy-bench (the pin), gzippy-isal bin b3d9435587186f27 (factor-8 reserve) / gzippy-native bin 9790e8f3. Hardened drivers scripts/bench/{oracle.sh,_oracle_guest.sh,parity.sh}. Host re-frozen (no_turbo=1, watchdog ttl 3600s). NO orphan processes. Verdict: plans/oracle-hardening-advisor-verdict.md.

## SUPERSEDED — PRIOR CURRENT STATE (2026-06-08, owner turn, branch reimplement-isa-l, HEAD 690941f3) — **PHASE 1 (source-map) + PHASE 2 (isolation prototype + measure) of the INLINE-ASM igzip fork RAN → DECISIVE NO-GO for the inline-asm transliteration approach (advisor disproof ×2, signed off pass-2). The partial-handoff inline-asm hot loop is WORSE than the LLVM-compiled pure-Rust loop AND worse than the naive scalar; the ONLY untested path is a full-kernel asm = re-writing ISA-L by hand, whose whole-system upside is ALREADY bounded by the ocl_cf 0.945× removal oracle. The user-fork is CORRECTLY TRIGGERED: accept ~0.86–0.945× pure-Rust, OR authorize a multi-session full-kernel asm — justified ONLY IF a low-T engine-bound CLOSABLE cell loses to rapidgzip (the engine is slack-masked at T8). NO production change (bench-only VAR_VII + plan docs; byte-exact preserved).**

THIS TURN (HEAD 690941f3): Executed the charter's TIERED Phase-1+Phase-2.
- **PHASE 1 SOURCE-MAP** (read-only subagent, cited): igzip `loop_block` asm:507-627. What the hand-asm does that LLVM does NOT emit from idiomatic Rust: F1 one-iteration-ahead literal-table gather across the back-edge (asm:540); F2 speculative dist-table gather before the lit-vs-len branch (asm:550-552); F3 unconditional flag-free SHLX/SHRX refill+consume, IN_BUFFER_SLOP=8 (asm:528-547,370); F4 all loop-carried state pinned in callee-saved GPRs (asm:108-136); C 8-byte speculative packed-literal store + advance-by-count (asm:518); D 16-byte MOVDQU + distance-doubling overlap copy (asm:603-627); E one slop-adjusted two-compare gate per multi-symbol iter (asm:488-512). VAR_V/VAR_VI already do C/D/E + a Rust PRELOAD; VAR_VII targets F1/F3/F4/C in `core::arch::asm!`.
- **PHASE 2 PROTOTYPE** (benches/engine_isolation.rs VAR_VII, committed 0c769c14→690941f3): a `core::arch::asm!` literal-run hot loop with read_in/ril/next_in/next_out pinned in registers, unconditional SHLX refill mirroring gzippy's Bits convention (bitsleft=len|56), 12-bit table gather, 8-byte speculative packed-literal store, one slop-gate. Exits to Rust to resolve each back-ref/long-code, then RE-ENTERS the asm. BYTE-EXACT: SHA_ALL_EQUAL=yes on all 5 swept clean silesia chunks vs VAR_I scalar AND VAR_III ISA-L (guest native). New GZIPPY_VII_COVERAGE counter: asm emits 57-99% of bytes (median ~0.65-0.74).
- **PHASE 2 MEASURE** (GUEST native x86_64, frozen host no_turbo=1 perf, taskset core 0, interleaved best-of-11, vs the ISA-L oracle): ISA-L 283 MB/s (1.000×) | VAR_VI (LLVM pure-Rust spec+BMI2+AVX) 168 MB/s (**0.594×** = the LLVM ceiling) | VAR_VII (inline-asm-reentry) 78 MB/s (**0.276×**, and only ~0.75× of the NAIVE SCALAR). TWO bracketing points: leading-run-only 0.55× (asm barely runs ≈ careful tail) and re-enter-per-symbol 0.28× (asm runs, handoff dominates). Rate FALLS monotonically as asm coverage rises ⇒ the per-symbol asm↔Rust re-entry (each `asm!` is an LLVM barrier; 300-460K/chunk, 4 regs spilled to `bits`) dominates.
- **FALSIFIER (pre-registered): PLATEAU/NO-GO** — inline asm does NOT close the 36ms; it regresses below the LLVM loop. The advantage ISA-L has is keeping the WHOLE loop (back-ref copy + dist gather + long code) in ONE register-pinned asm machine with zero handoff; any Rust-resident dist-decode forces a per-back-ref spill, and silesia is ~31% back-refs.
- **ADVISOR (independent disproof ×2, synchronous):** pass-1 (plans/phase2-inline-asm-advisor-verdict.md) BROKE the first cut (asm ran once/block over the leading run, 0.89% coverage ⇒ under-powered) → OWED a coverage counter + dominant-path asm. Owner delivered both. pass-2 (plans/phase2-inline-asm-advisor-verdict-pass2.md) tried 3 breaks, all FAILED → **NO-GO EARNED, escalation correctly triggered**; REVERSED its own pass-1 "build a full-kernel VAR_VIII before escalating" — the full-kernel upside is already bounded by the ocl_cf removal oracle (0.945× T8, engine-swap moves consumer wall ~5ms, ~6ms output floor survives full bootstrap removal), so VAR_VIII can only CAPTURE (not re-decide) the fork. SHARPENING: at T8 the engine is slack-masked → accept; but goal #1 is no-FFI parity across thread-count incl. LOW-T where the engine is NOT masked (0.55-0.60× ISA-L) — so the full-kernel asm is justified IFF a low-T engine-bound CLOSABLE cell loses to rapidgzip.

**SUPERVISOR GATE — INLINE-ASM FORK RESOLVED: NO-GO on the transliteration approach (decisive, advisor-signed). The remaining engine lever is ONLY a full-kernel asm (re-write ISA-L by hand) bounded by ocl_cf 0.945×. ESCALATE the user-fork: (A) ACCEPT ~0.86–0.945× pure-Rust and close the engine chapter, OR (B) authorize a multi-session full-kernel asm — but per the advisor this is justified ONLY if the owner first identifies a LOW-T, engine-bound, CLOSABLE cell that loses to rapidgzip (data the owner already has; T8 is slack-masked so it does NOT justify it). NEXT OWED before (B): a low-T (T1/T4) parity.sh whole-system cell vs rapidgzip to test whether any engine-bound cell is closable-and-losing.** GUEST: /root/gzippy (engine-bench checkout, bench overlaid from working tree); host auto-restored frozen (no_turbo=1 verified). Bench: benches/engine_isolation.rs VAR_VII (NV=10) + GZIPPY_VII_COVERAGE/GZIPPY_VII_CAREFUL_ONLY self-test knobs. Briefs/verdicts: plans/phase2-inline-asm-brief.md + plans/phase2-inline-asm-advisor-verdict{,-pass2}.md. NO orphan processes.

## SUPERSEDED — PRIOR CURRENT STATE (2026-06-08, owner turn, branch reimplement-isa-l, HEAD f98af1f) — **C2 NON-ENGINE RESIDUAL LOCALIZED + BOUNDED on the ocl_cf path (3 removal oracles, advisor-signed-off) → ALL C2 SUB-TERMS ARE FLAT-OR-SMALL: the 21ms is NOT a faithful low-risk scheduling tooth. Scheduling NULL, interior-reuse 1-chunk, bootstrap ≤8ms engine-class, dominant residual = the (prior-established) shared writev/bandwidth output floor. The "KEEP-GRINDING C2 instead of escalate" premise is SUBSTANTIALLY WEAKENED — C2 has no demonstrated wall-moving faithful tooth. NO production fix landed (the named lever is validated non-closeable, Rule-3 removals; output measurements + plan docs only). Host LOADED (loadavg ~4) — absolute split noisy, qualitative verdict robust.**

THIS TURN (HEAD f98af1f, branch reimplement-isa-l): Froze the host (neurotic /root/clock_freeze.sh freeze 5400 — no_turbo=1, gov=performance, cpu 1.4GHz, LXC 111/105 frozen; was THAWED on arrival, no_turbo=0). Built gzippy-isal @ f98af1f (bin_sha 2f8667619679b08c) via parity.sh. Re-confirmed ocl_cf = ISA-L engine oracle (engine-isolation kind) at HEAD: **0.395-0.419ms, 0.966-0.989× rg** (residual ~5-14ms; banked 0.945×/21ms reproduces structurally, smaller absolute on this host). production-isal (faithful two-phase) = 0.434ms 0.903×.

LOCALIZED (fulcrum_total trace + consumer_block_decompose.py, consumer tid=1, BOTH native + ocl_cf, conservation 0.03%): swapping the clean engine to ISA-L barely moves consumer_wall (0.3899→0.3851, ~5ms) and does NOT shrink DECODE_WAIT (0.2508→0.2463, ~64% of wall both). The consumer.writev (~0.12-0.13s) dominates the SERIAL bucket; the marker-resolve/publish spans are ~0ms (apply_window already off the consumer crit path = faithful to rg).

BOUNDED — THREE removal oracles on ocl_cf (Rule 3, pre-registered falsifiers, interleaved best-of-N, byte-exact A):
1. **PERFECT_OVERLAP** (dispatch-DEPTH head-of-line gap removal): A→B = **−0.2% FLAT** (warm_hit_frac 0.882) ⇒ project_confirmed_offset_prefetch_gap DEPTH reading DEAD.
2. **warm_miss cause** (advisor-owed): misses=2, Prefetch guard-rejects=**1** ⇒ the overshoot-tail interior-reuse mechanism fires on exactly 1 chunk, flat in both arms ⇒ TARGETING reading bounded to 1-chunk negligible (old memory's "4 stalls/3 overshoot" + 3 failed patches do not reproduce; near-zero payoff stands).
3. **GZIPPY_SEED_WINDOWS** bootstrap-removal (advisor-owed, masks-binder CEILING ⇒ upper bound): A−B = −2ms / +8ms (loaded, noisy) ⇒ marker-bootstrap ≤8ms engine-class; **ocl_cf+seedfull = 0.983-0.985× STILL NOT 1.0×** ⇒ ~6ms output-floor residual survives full bootstrap removal (fresh removal-based confirmation, not by-elimination).

ADVISOR (independent disproof, synchronous, ×2 passes — plans/c2-residual-disproof-verdict.md): pass-1 UPHELD-W-CAVEATS, owed the 2 measurements; pass-2 (both ran) **SIGNED OFF, "argued→removed", conclusion EARNED leg-by-leg**. 3 wording caveats applied to the brief ("shared" inherited not re-measured; "≤8ms" is a ceiling; loaded-host absolute-split caveat). Brief: plans/c2-residual-localization-brief.md.

**SUPERVISOR GATE — C2 located + bounded as non-closeable-faithfully (all sub-terms flat/small, advisor-signed). This is the prompt's case-4 outcome (validated removal bound + no faithful wall-moving mechanism), NOT the engine FFI fork. The fork input UPDATES: C2 is removed as the cheaper faithful alternative — the only remaining levers are (a) the engine (native→ocl_cf inline-asm fork, ~36ms) and (b) a small shared/irreducible output floor. This does NOT by itself escalate the engine fork (residual is small + partly shared, not provably engine-only at this loadavg). NEXT (owed): a QUIET-BOX re-measure to tighten the bootstrap/floor split + a turbo-freq confirmation of the 36/21 split, then the fork is a clean "inline-asm-or-accept-0.945-0.98×" decision.** GUEST: /root/gzippy-bench (GUEST_SRC pin), gzippy-isal bin sha 2f86676; drivers scripts/bench/{_oclcf_overlap_bound,_c2_advisor_owed}.sh + parity.sh/oracle.sh. NO orphan processes. Host left frozen (watchdog auto-restores at +5400s).

## SUPERSEDED — PRIOR CURRENT STATE (2026-06-08, owner turn, branch reimplement-isa-l, HEAD a884fa7b+wt) — **CLEAN ENGINE-RATE CEILING MEASURED + VALIDATED on the MATCHED comparator → engine plateau is REAL & validly bounded; STEP-2 techniques all done/inapplicable; but the FORK is NOT escalated — the binder is NOT yet engine-only (advisor: KEEP-GRINDING C2, do NOT escalate inline-asm yet).**

MERGE VALIDATED: lib suite 886 pass / 0 fail under GZIPPY_POISON_RESERVE=1 (the 3 pre-existing failures are fixed). parity.sh smoke OK. TOOLING FIXED (the merged parity.sh/oracle.sh could not build — 4 fixes, all in scripts only): (1) rsync `--delete-excluded`→`--delete` (macOS openrsync aborts on --delete-excluded + multi-exclude; --delete also preserves the dest target/ build cache); (2) RSYNC_PATHS missing `crates examples` (workspace member + coz_bench example → manifest parse fail); (3) _parity_guest.sh contamination-scrub unset GZIPPY_BIN (its own config var → set -u abort) — allowlisted; (4) fingerprint find-list += crates examples. Host frozen via neurotic /root/clock_freeze.sh (no_turbo=1, gov=performance, cores pinned 1.4GHz min==max, LXC 111/105 cgroup-frozen). A stale orphan test binary (PID 587818, /tmp/gz-ft-src, 970MB RSS) from a prior turn was killed before measuring.

THE DECISIVE NUMBERS (frozen host, MATCHED comparator = both tools → identical /dev/shm regular-file sink — NOT /dev/null per the output-reconciliation verdict; interleaved best-of-N min, sha-verified, window-absent-preserving):
| run | wall (min) | ratio vs rg |
|---|---|---|
| rapidgzip 0.16.0 (WITH_ISAL) | 376-383 ms | 1.000× |
| **ocl_cf** (gzippy + real ISA-L clean engine, copy-free) | **404 ms** | **0.945× = CEILING** |
| production native (pure-Rust) | 434-440 ms | 0.857-0.870× |

ocl_cf is the VALIDATED speed-UP ceiling (removal not slope; GZIPPY_ISAL_ENGINE_ORACLE on gzippy-isal). COVERAGE 14/0 verified (all 14 clean chunks ran ISA-L, 0 fallback). COVERAGE SYMMETRY ✓ (native routing flip_to_clean=12 finished_no_flip=4 window_seeded=2 == ocl_cf's 14). DRAIN SPLIT ✓ (advisor-owed GZIPPY_FOLD_NODRAIN/NOCRC, N=21): drain+CRC second-touch = ~0-1ms ⇒ the bulk clean tail is u8-DIRECT (decode_clean_into_contig, banked 0f5bc85b) ⇒ the **36ms native→ocl_cf gap is ~97% pure-Rust-vs-ISA-L SYMBOL RATE** on the SAME chunks. (The stale "ring-drain remains, upper bound only" comments at gzip_chunk.rs:871-875/996-999 were FIXED — doc-only/byte-transparent.)

ENGINE PLATEAU (advisor-VALIDATED, source+disasm-verified): all STEP-2 techniques done/inapplicable — #1 single-level L1 table DONE (12-bit, [u32;4096]=16KiB, igzip geometry); #2 BMI2 MAXED (1163 bzhi/pext idioms in native disasm); #3 static-Huffman NO HEADROOM (silesia L9 ~all dynamic); #4 FASTLOOP DONE (VAR_V pipeline in decode_clean_into_contig); #5 table-prefetch NO HEADROOM (table L1-resident already, no miss to prefetch — advisor corrected "structurally impossible"→"no miss to hide"); store TIE'd 7a; 0 bounds checks in hot loop. The 36ms is the intrinsic hand-asm(ISA-L)-vs-LLVM-codegen gap; the ONLY remaining pure-Rust lever is inline-asm transliteration of igzip's hot loop (highest-risk, NOT in STEP-2 list).

**SUPERVISOR GATE — FORK NOT ESCALATED (advisor disproof: KEEP-GRINDING C2). The fork is mis-framed as engine-bound: the engine is only ~63% of the gap (36ms). The C2 NON-ENGINE residual (ocl_cf 0.945×→rg 1.0× = 21ms) is present EVEN with real ISA-L FFI ⇒ it is NOT an engine problem — it's architecture/scheduling/marker-region = the faithful-port territory the charter prioritizes, LOWER-RISK, benefits BOTH goals, with a concrete located lever (project_confirmed_offset_prefetch_gap: 4 head-of-line stalls at confirmed offsets ≠ partition guess, ~40% of T8 wall). Escalate the inline-asm fork ONLY when the residual is provably engine-only (ocl_cf within noise of rg). NEXT (fresh arc, pre-register a falsifier): attack C2 — the confirmed-offset prefetch gap + marker-region/bootstrap scheduling, gated on the MATCHED comparator via parity.sh, bounded by removal-oracle. Owed at escalation: a turbo-freq confirmation of the split (1.4GHz pin exposes IPC cleanly but the absolute 36/21 split could shift at turbo).** NO production fix landed this turn (engine ceiling + plateau are measurements; the 4 script fixes + 2 doc-comment fixes are byte-transparent/tooling). GUEST: /root/gzippy-bench (the canonical pin; native build sha-OFF 028bd002…cb410f + gzippy-isal build for ocl_cf), drivers /root/gzippy-bench/fold_split2.sh; rg 0.16.0. Advisor verdicts: plans/clean-rate-ceiling-advisor-verdict{,-recheck}.md + plans/plateau-fork-advisor-verdict.md; briefs: plans/clean-rate-ceiling-brief.md, plans/fold-drain-split-result.md, plans/plateau-fork-brief.md. NO orphan processes.

## SUPERSEDED — PRIOR CURRENT STATE (2026-06-07, owner turn, branch reimplement-isa-l, HEAD f80294ae) — **PAGE-WARMTH CLEAN ORACLE RAN → MARKER-segment warmth sub-lever REFUTED (−12% fault ceiling, confound-independent); the CLASS is NOT refuted (advisor pass-3); TRIAGE to engine. Built+source-verified+measured (locked guest 10.30.0.199, taskset 0,2,4,6,8,10,12,14, interleaved N=15, byte-exact 028bd002…cb410f).** ROOT-CAUSE LOCALIZED via `perf record -e page-faults --call-graph fp` (symboled build): the dominant fault site is **`SegmentedU16::push_slice` = 44.52% of faults** (the u16 MARKER buffer write), NOT the clean-tail chunk.data (decode_huffman_body_resumable = 18.8%). The prior CURRENT STATE's "clean tail into a separate cold chunk.data" MISLOCATED the primary term. perf stat faults (file sink p8): gzippy 110,617 vs rg 55,790 = 1.98×. **THREE ORACLES (rule-#3 fault-removal): (a) GZIPPY_PREFAULT_ARENA (pre-touch+free per-worker arena) FAILED — faults went UP (193K/333K, rpmalloc handed the decode different pages); (b) GZIPPY_SLAB_ALLOC (≥3 MiB resident-retain) faults −10%, matched wall WORSE 0.971× (but its ≥3 MiB threshold MISSED the 128 KiB marker segments); (c) NEW GZIPPY_SLAB_THRESHOLD_KIB=64 (lowers resident-retain to capture the 128 KiB u16 marker segments, sub-MiB→128 KiB granularity, the FIRST oracle to target the 44% fault site): faults 110,619→97,377 (−12%, STILL 1.77× rg, FAR from the 55K floor), matched wall gz_s64 0.1314 vs gz_null 0.1324 = 1.008× FLAT.** ADVISOR DISPROOF ×3 (synchronous, plans/page-warmth-rootcause-advisor-verdict.md): the −12% CEILING is confound-independent and IS the refutation of the MARKER sub-lever (a perfect zero-overhead version still only retains marker segments, can't reach the floor); the FLAT matched wall proves NOTHING (underpowered TIE — max plausible warmth win single-digit ms is 5-10× below the 13-18ms harness spread; rule 5/7). **CORRECTED (do NOT overclaim): "page-warmth refuted as a CLASS" is NOT earned — the 211 MiB OUTPUT materialization (~56% of faults, the site rg avoids via windowed/recycled append DecodedData.hpp:344-388) was NEVER warmth-oracle'd.** rg faults half NOT only via ISA-L (engine) but via its windowed/recycled MATERIALIZATION (data structure — pure-Rust-portable, NOT forbidden by goal #1; = the rule-7a worker-side recycle already owed; the prior 3.26× DTLB regression was FRESH segmentation, RECYCLED warm-TLB windows are a DIFFERENT unrun experiment). rapidgzip --verbose: rg has the SAME 34.5% replaced markers (73.1M) + 0.0338s apply-window narrow ⇒ the u16 footprint is NOT gzippy-unique. **SUPERVISOR GATE — page-warmth MARKER sub-lever dead; CLASS not refuted; NO production fix landed (3 measurement knobs KEPT 7a, byte-exact, OFF-default, NOT promoted: GZIPPY_PREFAULT_ARENA, GZIPPY_SLAB_THRESHOLD_KIB). NEXT (triage, advisor-endorsed): the CLEAN-ONLY ENGINE ORACLE (the 2.3× clean-rate gap ≫ warmth's hidden few-ms, owed anyway — project_pregate_placement_is_dominant_lever); then sched slow-inject if engine bounds short. STILL OWED for page-warmth (rule-7a convergence, the untested 56%): a recycled-window oracle on the OUTPUT materialization (faithful DecodedData.hpp:344-388, reused 128 KiB append windows, warm-TLB).** GUEST: /tmp/gz-ft-src build (native, sha 028bd002…cb410f OFF) + /tmp/gz-ft-sym/release/gzippy (symboled, faults profiling); drivers /tmp/measure_devnull.sh + /root/gzippy/scripts/measure.sh; rg 0.16.0. NO orphan processes. Briefs: plans/page-warmth-{rootcause,final}-brief.md; verdicts: plans/page-warmth-rootcause-advisor-verdict.md.

## SUPERSEDED — PRIOR CURRENT STATE (2026-06-07, owner turn, branch reimplement-isa-l, HEAD 20084c91) — **OUTPUT-BINDER RECONCILIATION (matched /dev/null comparator + rg output exposure measured): the prompt's headline ("removing output ≈ parity") is a Rule-6 MISPAIRING (gz-output-REMOVED 0.131 vs rg-output-PRESENT 0.130) and is REFUTED. There are TWO T8 binders, NOT one. Matched comparator (BOTH tools→/dev/null, locked guest, interleaved): gz_null 0.1324 vs rg_null 0.1148 = 1.16× — a residual that SURVIVES output removal on both sides. SPLIT (output exposure = file−null, MEASURED for both): gzippy 35.3ms, rg 15.7ms ⇒ rg ALSO pays a serial output exposure; of gzippy's 35.3ms, ~15.7ms is the SHARED memory-bandwidth floor (rg pays it too, NOT a gzippy deficit) and ~19.6ms is gzippy-specific excess; PLUS a separate ~17.6ms engine+sched residual. The output-overlap lever is REFUTED as the path: (a) NON-FAITHFUL — rg's writeFunctor is INLINE-SYNCHRONOUS in the consumer read() loop (ParallelGzipReader.hpp:621, verified first-hand), NO background thread; gzippy's output_writer.rs OverlapWriter is a structural DIVERGENCE rejectable under the bias guardrails. (b) SUB-PARITY + the 0.88× "ceiling" was a construction — Linux write_null skips copy_from_user so file−null bundles the COLD-SOURCE read; the "output excess" is largely the SAME cold-page tax as the residual, and overlap (a timing change) cannot fix a cold copy. Only the shipped writer's measured 0.81× is real. The UNIFIED ROOT of both binders = PAGE WARMTH: perf stat on the arena-wired binary shows gzippy 110,301 page-faults / 894ms task-clock vs rg 58,211 / 622ms = ~1.9× faults DESPITE the rpmalloc arena already wired (pure-rust-inflate→rpmalloc-caches→arena-allocator; U8/U16=Vec<_,RpmallocAlloc>); faults are topology-invariant (~110K under overlap AND manual-pool) ⇒ intrinsic to gzippy's per-chunk allocation FOOTPRINT. The "u16→u8 second pass is gzippy's deficit" thesis is ALSO FALSE — rg runs the SAME applyWindow narrowing (DecodedData.hpp:164,306, MarkerVector=FasterVector<u16>) and does it IN PLACE reusing the u16 buffer's pages (DecodedData.hpp:344-388 reinterpret_cast+reusedDataBuffers); gzippy's markered narrow IS in-place (apply_window.rs resolve_markers_u16) but the CLEAN tail decodes into a SEPARATE chunk.data ⇒ the extra alloc+faults. Advisor disproof synchronous (plans/output-reconciliation-advisor-verdict.md), all kill-shots vendor-source-verified. NO production fix landed (overlap writer stays OFF-by-default, NOT promoted; KEPT 7a). T8 is NOT at candidate-parity. NEXT (faithful, gated, do NOT start without supervisor): the PAGE-WARMTH root — faithful in-place clean-tail narrowing reusing the marker buffer's pages + cross-thread arena warmth (rg DecodedData.hpp:344-388), gated on the MATCHED gz_null-vs-rg_null comparator (NOT skip-vs-rg-file), pre-registered falsifier. Do NOT re-open the output-overlap fork. GUEST: /tmp/gz-ft-src (native, sha 028bd002…cb410f), drivers /tmp/{_split_driver,measure_devnull}.sh + /root/gzippy/scripts/measure.sh, rg 0.16.0. NO orphan processes.**

## SUPERSEDED — PRIOR CURRENT STATE (2026-06-07, owner turn, branch reimplement-isa-l) — **OUTPUT IS THE T8 BINDER (fulcrum_total + validated writev-removal oracle): removing gzippy's serial output writev moves T8 0.79×→~0.98× rg ⇒ OUTPUT (the serial 211 MiB page-cache materialization on the in-order consumer) is the SINGLE LARGEST T8 binder, NOT the engine. The prior "0.135× engine table-load + 0.075× placement" split is REVISED — the engine is the T1 binder, SLACK-MASKED at T8 (parallelized ~8×); the un-parallelizable serial output copy is the Amdahl T8 tail. The instant-feed discriminator REFUTED the advisor's "feed-rate-masked" phantom (output exposure stayed 35→31ms when the engine sped up 38ms ⇒ engine-INDEPENDENT). FAITHFUL TECHNIQUE LANDED (byte-exact, rule-7a KEEP): background overlap writer (mirrors rg writeFunctor) takes the writev off the consumer crit-path → +3.7-6% sign-stable over 5 passes (0.79×→~0.81× rg), TIE-by-spread, partial (~6ms of the ~20ms ceiling; the rest is the irreducible ~14ms memory-bandwidth floor rg also pays).**
...
- **THE SPLIT (locked guest 10.30.0.199, taskset 0,2,4,6,8,10,12,14, gov=perf, interleaved measure.sh, sha-OK every run):** 6-pass 3-way old / new_off / new_contig = 0.634 / 0.674 / 0.717× rg means, MONOTONIC every pass. copy#1 (ring→u8buf alloc+byteloop eliminated) = +0.040×; copy#2/3/grow (pending_clean middle-man + append_clean + regrow eliminated) = +0.043×. The FIRST control (pending_clean only, under-reserved) TIE'd — the advisor caught that I'd left the dominant ring→u8buf per-block copy untouched; removing it MOVED the wall, proving a free cadence component had been mis-booked as intrinsic.
...
- **APPLES-TO-APPLES --verbose (locked guest 10.30.0.199, T8 unseeded, taskset 0,2,4,6,8,10,12,14, gov=perf):** gzippy decodeBlock SUM **0.805-0.831s** vs rg **0.4995s = 1.61-1.66×**. Per-engine: gzippy marker bootstrap body **0.323s** (73.0M @ 226-235 MB/s) vs rg "custom inflate" **0.4748s** ⇒ gzippy marker is 0.68× = **FASTER**. gzippy clean tail **≈0.48s** (≈139M, ≈290 MB/s, by subtraction) vs rg "ISA-L" **0.2065s** ⇒ **≈2.3× SLOWER**. gzippy routing: flip_to_clean=12, finished_no_flip=4, window_seeded=2 (17 chunks). markers 34.5% both (gzippy 73.0M ≈ rg 73.1M).
...
- **FIRST-HAND VERBOSE (locked guest 10.30.0.199 double-ssh, 16c gov=perf, gzippy-isal native synced to HEAD f1aceee1, sha 028bd002…cb410f every cell, T8):**
...
- **REMOVE-AND-MEASURE (NOT the SUM, per advisor Q4): locked guest 10.30.0.199 double-ssh, 16c gov=performance turbo-on, taskset 0,2,4,6,8,10,12,14, T8, measure.sh interleaved N=11, RAW=68229982, sha-verified=OK every run. base(WITH merge) vs mergefix(REMOVED), both gzippy-isal native target-cpu=native:**
...
- **OWED apply_window measurement (locked guest 10.30.0.199 double-ssh, 16c gov=perf turbo-on load ~1.0, taskset -c 0,2,4,6,8,10,12,14, T8, RAW=68229982, sha 028bd002…cb410f EVERY run, /tmp/gzbuild-isal gzippy-isal native, measurement-only sub-step timers added byte-exact (NOT committed), 3 runs):**
...
- **MEASURED (locked guest 10.30.0.199 double-ssh, 16c gov=perf turbo-on load 1.3-2.0, measure.sh interleaved N=11 CPUS=0,2,4,6,8,10,12,14 RAW=68229982 sha-OK=028bd002…cb410f every cell, 2 runs):**
...
- **CAUSAL PERTURBATION (this turn; new GZIPPY_SLOW_MARKER_MODE u16-path knob, commit fb3baec0;
  ...
  10.30.0.199 double-ssh, 16c gov=perf, measure.sh interleaved sha-OK, RAW=211968000, T8
...
- Whole-system wall (locked guest trainer=10.30.0.199 via -J neurotic, 16c gov=performance
