...
## ====== NIGHT31 (2026-06-19, branch kernel-converge-A, IN PROGRESS) — ISOLATED KERNEL-vs-KERNEL A/B (resolve WEAK NIGHT26 attribution vs STRONG removal-oracles). PLAN/CHECKPOINT. ======
GOAL: locate the +0.9 cyc/B residual with STRONG isolated-causal evidence. ARM A = gz production CLEAN-CONTIG kernel `Block::decode_clean_into_contig` (= run_contig + its inseparable resumable glue, NIGHT27's definition of gz's kernel) decoding ONE real silesia dynamic block, looped to >=256 MiB total, tables (LUTs+DistTable) built ONCE before the timer (block_huffman_luts_ready stays true across iterations). ARM B = igzip `decode_huffman_code_block_stateless_04` (the AVX2/BMI2 hand-asm, exported @0x38be0 in libisal.so.2; CONFIRMED via `nm -D`) decoding the SAME block body, igzip tables built ONCE via one bounded `isal_inflate` header-parse before the timer, kernel call timed alone. DESIGN RATIONALE: the brief's "one huge dynamic block" requirement is to amortize per-block table-build to ~0; looping ONE REAL silesia block achieves identical amortization with a REAL symbol mix and ZERO hand-rolled-encoder risk (avoids an unfaithful synthetic block). Harness = a gzippy example (required-features pure-rust-inflate,isal-compression). GATE-0: assert _04 selected (call _04 DIRECTLY — exported — removing dispatch ambiguity); A==B==flate2 byte-identical (non-inert); in-RAM dst both arms (same sink); conservation. MEASURE: cyc/B+IPC+instr/B via perf stat around ONLY the kernel loop, paired interleaved N>=11, CI-disjoint. Intel-LXC ONLY → NOT-YET-LAW; AMD owed. Resume point: harness example being authored (examples/kernel_ab.rs); guest run pending. Box: no stray PIDs (none spawned yet).
