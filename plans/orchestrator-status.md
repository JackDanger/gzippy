...
ENVIRONMENT TRUTH (corrects stale MEMORY): neurotic reachable `ssh -J REDACTED_IP root@REDACTED_IP`;
fixed local ~/.ssh/config (added HostName REDACTED_IP to `Host neurotic` so guest.env JUMP=neurotic
...
(host=neurotic REDACTED_IP), NOT the guest; no_turbo=1 already.
