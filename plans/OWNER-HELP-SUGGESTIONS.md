...
the full chain through the `ssh neurotic 'ssh 10.30.0.199 …'` double-hop:
...
- guest build is re-typed each time, e.g.
  `timeout 600 ssh neurotic 'ssh 10.30.0.199 "cd /tmp/gz-ft-src && RUSTFLAGS=\"-C target-cpu=native\" cargo build --release --no-default-features --features pure-rust-inflate …"'`
...
**Artifact.**
- `scripts/bench/guest.env` — single source of truth: `GUEST=10.30.0.199`,
...
- `plans/GUEST.md` — the cheat-sheet: the canonical double-hop form
  (`ssh -J neurotic root@10.30.0.199 …` is shorter and already used in newer turns —
...
`ssh -J neurotic root@10.30.0.199` (the shorter, newer form), per the "no wedged channel"
